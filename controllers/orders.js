...
async function getMySponsorships(req, res) {
  try {
    ...
    const includeRelations = req.query.include ? 
      req.query.include.split(',').filter(rel => 
        ['project', 'plan', 'shipping', 'invoice'].includes(rel)
      ) : 
      ['project', 'plan'];

    // 使用 QueryBuilder 提供更好的查詢控制
    const queryBuilder = sponsorshipRepo.createQueryBuilder('sponsorship')
...
async function getMySponsorshipsWithFilters(req, res) {
  try {
    ...
    const order = ['ASC', 'DESC'].includes(sort_order.toUpperCase()) ? sort_order.toUpperCase() : 'DESC';
